...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.LongTensor(convert_one_line(text, max_seq_length=220, tokenizer=self._tokenizer, split_point=self._split_point)), lens, target, weight
...
def main():
    ...
    if args.mode == 'train' or args.mode == "train_all":
        ...
        if args.model in ["bert-base-uncased", "bert-base-cased", "mybert", "gpt2",'mybert-base-cased','mybert-base-uncased']:
            NUM_LAYERS = 12
...
def validation(model: nn.Module, criterion, valid_df, valid_loader, args, save_result = True, progress = True) -> Dict[str, float]:
    run_root = Path('../experiments/'+args.run_root)
    model.eval()
